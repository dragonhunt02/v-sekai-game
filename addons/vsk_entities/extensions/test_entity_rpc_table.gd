...
@rpc("any_peer") func spawn_ball(p_entity_callback_id) -> void:
	if NetworkManager.is_session_master():
		...
		if get_remote_sender_id() == NetworkManager.get_current_peer_id():
			session_puppet_spawn.emit(p_entity_callback_id)

@rpc("any_peer") func spawn_prop(p_entity_callback_id, p_prop_scene) -> void:
	if NetworkManager.is_session_master():
		session_master_spawn.emit(get_remote_sender_id(), p_entity_callback_id, p_prop_scene)
	else:
		if get_remote_sender_id() == NetworkManager.get_current_peer_id():
			session_puppet_spawn.emit(p_entity_callback_id, p_prop_scene)
