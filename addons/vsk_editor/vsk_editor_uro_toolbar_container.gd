...
func _attempt_service_access() -> void:
	...
	if not get_tree().node_added.is_connected(_node_added):
		if not VSKUtils.assert_ok(get_tree().node_added.connect(_node_added),
...
func _ready() -> void:
	if not get_tree().edited_scene_root:
		if not VSKUtils.assert_ok(account_options_button.get_popup().id_pressed.connect(_account_id_option_pressed),
