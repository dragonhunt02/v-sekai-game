...
func sign_in(p_game_service: SarGameService, p_sign_in_data: Dictionary) -> void:
	...
	if not VSKUtils.assert_equal(_service, null, "func sign_in: '_service' already exists")
		return
	if not VSKUtils.assert_equal(_request, null, "func sign_in: '_request' already exists")
		return
