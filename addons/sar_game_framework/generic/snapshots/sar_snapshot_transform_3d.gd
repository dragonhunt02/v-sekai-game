...
func encode_snapshot(p_stream_peer: StreamPeer, p_bit_offset: int) -> StreamPeer:
	...
	if not VSKUtils.assert_ok(resize_result, "SarTransform3DSnapshot: Could not resize PackedByteArray."):
		return null
	...
	if not VSKUtils.assert_ok(_result[0], "SarTransform3DSnapshot.encode_snapshot: Unexpected error while sending snapshot data."):
		return null
	if not VSKUtils.assert_equal(_result[1], ceil(float(get_size()) / BITS), "SarTransform3DSnapshot.encode_snapshot: Did not send expected number of bytes in snapshot."):
		return null
