...
func encode_snapshot(p_stream_peer: StreamPeer, p_bit_offset: int) -> StreamPeer:
	...
	if not VSKUtils.assert_ok(_result[0], "SarFixedSizeSnapshot.encode_snapshot: Unexpected error while sending snapshot data."):
		return null
	if not VSKUtils.assert_equal(_result[1], ceil(float(get_size()) / BITS), "SarFixedSizeSnapshot.encode_snapshot: Did not send expected number of bytes in snapshot."):
		return null
