...
func _input(p_event: InputEvent) -> void:
	if not Engine.is_editor_hint() and is_multiplayer_authority():
		...
		if vessel:
			...
			if not VSKUtils.assert_true(input_component, "SarSoulPlayerCommandsComponent: _input_component is not available"):
				return
...
func _physics_process(delta: float) -> void:
	if not Engine.is_editor_hint() and is_multiplayer_authority():
		...
		if vessel:
			var input_component: SarGameEntityComponentVesselInput = (vessel.get_game_entity_interface() as SarGameEntityInterfaceVessel3D).get_input_component()
			if not VSKUtils.assert_true(input_component, "SarSoulPlayerCommandsComponent: input_component is not available."):
				return
