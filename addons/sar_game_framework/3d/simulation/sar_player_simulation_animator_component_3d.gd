...
func _ready() -> void:
	if not Engine.is_editor_hint():
		if not VSKUtils.assert_true(simulation, "SarSimulationComponentAnimator3D: simulation is not available"):
			return
		
		_model_component = simulation.game_entity_interface.get_model_component()
		if not VSKUtils.assert_true(_model_component, "SarSimulationComponentAnimator3D: _model_component is not available"):
			return
