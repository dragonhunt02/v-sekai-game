...
func _on_pre_movement(p_delta: float, _velocity: Vector3) -> void:
	...
	if not VSKUtils.assert_true(game_entity_interface, "SarSimulationComponentAuxiliaryMotion3D._on_pre_movement: game_entity_interface is not available"):
		return
	
	var movement_component = game_entity_interface.get_movement_component()
	if not VSKUtils.assert_true(movement_component, "SarSimulationComponentAuxiliaryMotion3D._on_pre_movement: movement_component is not available"):
		return
