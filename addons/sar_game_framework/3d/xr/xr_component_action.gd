...
func _ready() -> void:
	...
	if controller:
		if not VSKUtils.assert_ok(controller.button_pressed.connect(_button_pressed),
			"Could not connect signal 'controller.button_pressed' to '_button_pressed'"):
			return
		if not VSKUtils.assert_ok(controller.button_released.connect(_button_released),
