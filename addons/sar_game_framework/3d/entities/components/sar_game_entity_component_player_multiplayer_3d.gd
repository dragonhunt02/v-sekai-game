...
func _setup_authority() -> void:
	if not VSKUtils.assert_true(game_entity, "SarGameEntityComponentPlayerMultiplayer3D: game_entity is not available"):
		return
	
	var game_session_manager: SarGameSessionManager = get_tree().get_first_node_in_group("game_session_managers")
	if not VSKUtils.assert_true(game_session_manager, "SarGameEntityComponentPlayerMultiplayer3D: game_session_manager is not available"):
		return
