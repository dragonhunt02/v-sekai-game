...
func _bind_simulation_node() -> void:
	if _simulation_node:
		...
		if not game_entity.transform_changed.is_connected(_simulation_node._on_game_entity_transform_changed):
			if not VSKUtils.assert_ok(game_entity.transform_changed.connect(_simulation_node._on_game_entity_transform_changed),
		...
		if not game_entity.transform_pre_update.is_connected(_simulation_node._on_game_entity_transform_pre_update):
			if not VSKUtils.assert_ok(game_entity.transform_pre_update.connect(_simulation_node._on_game_entity_transform_pre_update),
		...
		if not game_entity.transform_post_update.is_connected(_simulation_node._on_game_entity_transform_post_update):
			if not VSKUtils.assert_ok(game_entity.transform_post_update.connect(_simulation_node._on_game_entity_transform_post_update),
		...
		if not model_component.model_changed.is_connected(_simulation_node._on_character_model_component_model_changed):
			if not VSKUtils.assert_ok(model_component.model_changed.connect(_simulation_node._on_character_model_component_model_changed),
		...
		if not model_component.model_pre_change.is_connected(_simulation_node._on_character_model_component_pre_model_changed):
			if not VSKUtils.assert_ok(model_component.model_pre_change.connect(_simulation_node._on_character_model_component_pre_model_changed),
		...
		if not vessel_movement_component.pre_movement.is_connected(_simulation_node._on_vessel_movement_component_pre_movement):
			if not VSKUtils.assert_ok(vessel_movement_component.pre_movement.connect(_simulation_node._on_vessel_movement_component_pre_movement),
		...
		if not vessel_movement_component.post_movement.is_connected(_simulation_node._on_vessel_movement_component_post_movement):
			if not VSKUtils.assert_ok(vessel_movement_component.post_movement.connect(_simulation_node._on_vessel_movement_component_post_movement),
		...
		if not vessel_movement_component.movement_complete.is_connected(_simulation_node._on_vessel_movement_component_movement_complete):
			if not VSKUtils.assert_ok(vessel_movement_component.movement_complete.connect(_simulation_node._on_vessel_movement_component_movement_complete),
		...
		if not vessel_posession_component.possessed_by_soul.is_connected(_simulation_node._on_vessel_possession_component_soul_changed):
			if not VSKUtils.assert_ok(vessel_posession_component.possessed_by_soul.connect(_simulation_node._on_vessel_possession_component_soul_changed),
