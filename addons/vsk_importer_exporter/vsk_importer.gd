...
static func scan_ref_node_tree(
	p_ref_branch: RefNode, p_canvas: bool, p_validator: RefCounted
	...
	if p_ref_branch.class_str == "Instanced":
		if p_validator.is_path_an_entity(p_ref_branch.instance_path):
			...
			return {
				...
				"info":
				"Attempted to instance a none entity on node '{node_path}'".format(
	...
	for property in p_ref_branch.properties:
		...
		if property_value is NodePath:
			var ref_node_path_target: RefNode = get_ref_node_from_relative_path(
				p_ref_branch, property_value
			)
			if (
				ref_node_path_target == null
				and property_name != "vskeditor_preview_camera_path"
				and property_name != "skeleton"
			):
				return {
					"code": ImporterResult.UNSAFE_NODEPATH,
					"info":
					(
						"Unsafe node path on node: '{node_path}', property:'{property}', path:'{value}'"
						. format(
							{
								"node_path": create_path_from_root_to_node(p_ref_branch),
			...
			if property_name == "script":
				if is_instance:
					return {
						...
						"info":
						(
							"""Script assigned to instance node
							node: '{node_path}',
				...
				if p_ref_branch.parent == null:
					...
					if !p_validator.is_script_valid_for_root(
						property_value, p_ref_branch.class_str
					):
						return {
							"code": ImporterResult.INVALID_ROOT_SCRIPT,
							"info":
							(
								"Invalid script for root node: '{node_path}', script_path: '{script_path}'"
								. format(
									{
										"node_path": create_path_from_root_to_node(p_ref_branch),
										"script_path": property_value.resource_path
					...
					if p_validator.is_valid_canvas_3d_anchor(
						property_value, p_ref_branch.class_str
					):
					...
					# Check if this object is a canvas
						...
						if p_canvas:
							return {
								...
								"info":
								(
									"Recursive canvas detected in '{node_path}'"
									. format(
										{
											"node_path":
											create_path_from_root_to_node(p_ref_branch),
										}
									)
								)
							}
						...
						if !p_validator.is_script_valid_for_children(
							property_value, p_ref_branch.class_str
						):
							return {
								...
								"info":
								(
									"Invalid script for for child node: '{node_path}', script_path: '{script_path}'"
									. format(
										{
											"node_path":
											create_path_from_root_to_node(p_ref_branch),
	...
	if !skip_type_check:
		if !p_validator.is_node_type_string_valid(p_ref_branch.class_str, p_canvas):
			return {
				...
				"info":
				"Invalid node class '{class_name}' in node '{node_path}'".format(
					{
						"node_path": create_path_from_root_to_node(p_ref_branch),
	...
	for animation in animations:
		...
		if !animation_player_ref_node:
			return {
				...
				"info":
				(
					"Invalid root node for animation player '{node_path}', with path {root_path}"
					. format(
						{
							"node_path": create_path_from_root_to_node(p_ref_branch),
		...
		for i in range(0, track_count):
			...
			var track_ref_node: RefNode = get_ref_node_from_relative_path(
				animation_player_ref_node, track_path
			)
			if !track_ref_node:
				return {
					...
					"info":
					(
						"Invalid track_path for animation player '{node_path}', with path {track_path}"
						. format(
							{
								"node_path": create_path_from_root_to_node(p_ref_branch),
			...
			if track_type_int == Animation.TYPE_METHOD:
				return {
					...
					"info":
					(
						"Attempted to implement method track on '{node_path}', with path {track_path}.\nNot method tracks are current allowed until they can be implemented safely"
						. format(
							{
								"node_path": create_path_from_root_to_node(p_ref_branch),
								"track_path": str(track_path)
				...
				if !p_validator.validate_value_track(
					track_path.get_concatenated_subnames(), track_ref_node.class_str
				):
					return {
						"code": ImporterResult.INVALID_VALUE_TRACK,
						"info":
						(
							"Attempted to implement value track on '{node_path}', with path {track_path}"
							. format(
								{
									"node_path": create_path_from_root_to_node(p_ref_branch),
									"track_path": str(track_path)
	...
	for child_ref_node in p_ref_branch.children:
		var result: Dictionary = scan_ref_node_tree(
			child_ref_node, children_belong_to_canvas, p_validator
		)
	...
	for snode in p_node_data_array:
		...
		if p_names.size() > snode.name_id - 1:
			...
			for property in snode.properties:
				...
				ref_node.properties.push_back({"name": property_name, "value": property_value})
		...
		if parent_id < 0 or parent_id == NO_PARENT_SAVED:
			...
			if parent_id & FLAG_ID_IS_PATH:
				...
				if p_node_paths.size() > idx - 1:
					...
					var parent_node: RefNode = get_ref_node_from_relative_path(
						root_ref_node, node_path
					)
					if parent_node:
...
static func sanitise_packed_scene(
	...
	if p_packed_scene == null:
		return {
			"packed_scene": null, "result": {"code": ImporterResult.NULL_PACKED_SCENE, "info": ""}
		}
	...
	if node_count > 0:
		...
		for _i in range(0, node_count):
			...
			if snode_reader.idx != -1:
				...
				result = {
					"code": ImporterResult.READ_FAIL, "info": "Failed to read name_id/index_id"
				}
				break
			...
			if snode_reader.idx != -1:
				...
				if group_count > 0:
					result = {
						"code": ImporterResult.HAS_NODE_GROUPS,
						"info": "Packed scene contains node groups"
					}
					break
			else:
