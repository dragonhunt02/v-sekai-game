...
func _on_focus_entered():
	...
	button_node.pivot_offset = self.get_parent().size / 2
	...
	active_tween.tween_property(button_node,"scale", Vector2(0.9, 0.9), trans_time)
	active_tween.chain() \
	.tween_property(button_node,"scale", Vector2(1.0,1.0), trans_time)
